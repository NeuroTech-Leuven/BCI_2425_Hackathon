# -*- coding: utf-8 -*-
"""
Created on Sat Nov  9 13:52:05 2024
...
class MindfulnessApp:
    def __init__(self, master):
        ...
        # Generate a new pattern for the blobs using Gaussian functions
        self.x = np.linspace(0, 10, 100)
        self.y = np.linspace(0, 10, 100)
        self.X, self.Y = np.meshgrid(self.x, self.y)

        # Create a pattern using Gaussian functions
        self.Z = (np.exp(-((self.X - 5)**2 + (self.Y - 5)**2) / 2) +
                   np.exp(-((self.X - 3)**2 + (self.Y - 7)**2) / 2) +
                   np.exp(-((self.X - 7)**2 + (self.Y - 3)**2) / 2))

        # Create an initial plot
        self.image = self.ax.imshow(self.Z, cmap=cmap, interpolation='bilinear', alpha=0.7, norm=Normalize(vmin=0, vmax=1))
