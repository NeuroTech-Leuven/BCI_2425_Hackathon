# -*- coding: utf-8 -*-
"""
Created on Sat Nov  9 13:52:12 2024
...
class MindfulnessApp:
    def __init__(self, master):
        ...
        # Generate a new complex pattern for the blobs
        self.x = np.linspace(0, 10, 100)
        self.y = np.linspace(0, 10, 100)
        self.X, self.Y = np.meshgrid(self.x, self.y)

        # Create a complex pattern using a combination of sine waves, Gaussian functions, and noise
        self.Z = (np.sin(self.X) * np.cos(self.Y) +
                   np.exp(-((self.X - 5)**2 + (self.Y - 5)**2) / 2) +
                   0.2 * np.random.rand(*self.X.shape))  # Adding random noise

        # Normalize the pattern to ensure it fits within a specific range
        self.Z = (self.Z - np.min(self.Z)) / (np.max(self.Z) - np.min(self.Z))  # Normalize to [0, 1]

        # Create an initial plot
        self.image = self.ax.imshow(self.Z, cmap=cmap, interpolation='bilinear', alpha=1, norm=Normalize(vmin=0, vmax=1))
        self.ax.set_title(f'Mindfulness Level: {self.mindfulness_level}%')
        self.ax.axis('on')  # Turn off the axis for a cleaner look
